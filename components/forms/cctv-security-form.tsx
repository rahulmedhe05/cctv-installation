...
interface CCTVSecurityFormProps {
  area?: string;
  keyword?: string;
}

export function CCTVSecurityForm({ area, keyword }: CCTVSecurityFormProps) {
  ...
  const propertyTypes = ["Home", "Shop", "Office", "Factory"];
  const securityRequirements = ["CCTV", "Biometric", "Fire Alarm", "Full Security System"];
  const installationTypes = ["New", "Upgrade"];

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setIsSubmitting(true);

    let message = `📹 *CCTV & SECURITY ENQUIRY*\n\n`;
    message += `👤 *Name:* ${formData.fullName}\n`;
    message += `📱 *Mobile:* ${formData.mobile}\n`;
    message += `🏢 *Property Type:* ${formData.propertyType}\n`;
    message += `🔒 *Security Requirement:* ${formData.securityRequirement}\n`;
    message += `🔧 *Installation Type:* ${formData.installationType}\n`;
  ...
  const whyChooseUs = [
    ...
    "Authorized Dealer - Hikvision, CP Plus, Dahua",
        ...
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-8 md:mb-12">
            ...
            <p className="text-gray-600 text-lg max-w-2xl mx-auto">
              Protect your property with professional CCTV & security solutions
              {area && ` in ${area}`}
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-8 items-start">
            ...
            <div className="bg-white rounded-2xl p-6 md:p-8 shadow-xl border border-gray-100">
              <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {securityRequirements.map((req) => (
                      <button
                        key={req}
                        type="button"
                        onClick={() => setFormData({ ...formData, securityRequirement: req })}
                        className={`p-3 rounded-lg border text-sm font-medium transition-all ${
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {installationTypes.map((type) => (
                      <button
                        key={type}
                        type="button"
                        onClick={() => setFormData({ ...formData, installationType: type })}
                        className={`p-3 rounded-lg border text-sm font-medium transition-all ${
                ...
                <Button
                  ...
                  className="w-full bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white font-semibold py-6 text-lg"
                >
            ...
            <div className="space-y-6">
              <div className="bg-gradient-to-br from-red-600 to-rose-600 rounded-2xl p-6 text-white">
              ...
              <div className="bg-green-600 rounded-2xl p-6 text-white">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <a
                      href="https://wa.me/916353583148?text=Hi, I need CCTV/security installation"
                      target="_blank"
              ...
              <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
                <h3 className="text-xl font-bold mb-4 text-red-700">Why Choose Us?</h3>
